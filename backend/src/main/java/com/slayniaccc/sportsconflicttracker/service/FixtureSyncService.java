...
public FixtureSyncService(FixtureRepository fixtureRepository, TeamRepository teamRepository){
    ...
    public void syncNbaFixtures(String apiKey) {
        ...
        for (var game : response.data()) {
          if (fixtureRepository.findByExternalId(String.valueOf(game.id())).isPresent()) {
            continue;
          }
               TeamEntity home = teamRepository.findByExternalId(String.valueOf(game.home_team().id()))
            .orElseThrow(() -> new IllegalStateException("Unknown home team: " + game.home_team().id()));
     TeamEntity away = teamRepository.findByExternalId(String.valueOf(game.visitor_team().id()))
